describe('Create an Engagement', () => {
  it('Get Token', () => {
    ...
    cy.login(Cypress.env('SSO_URL'), Cypress.env('SSO_USER') , Cypress.env('SSO_PASSWORD'))
    ...
    cy.get('[data-cy=new_customer_name]').type('e2e')
      .get('[data-cy=new_engagement_name]').type('cypressio')
      .get('[data-cy=createNewEngagement]').click()
    
    cy.wait('@createCheck').should('have.property', 'status', 201)

    cy.get('input[name=location]').should('have.value', '').type('Katmandu, Nepal')
      .get('input[name=start_date]').type('2020-10-25')
      .get('input[name=end_date]').type('2020-12-25')
      .get('textarea[name=description]').type('Herp derpsum derp herpy le nerpy terp jerpy derpy.')
    ...
    cy.get('[aria-label="Cloud provider region"]').select('us-east-1').should('have.value', 'us-east-1')
      .get('[aria-label="OpenShift Version"]').select('v4.4').should('have.value', '4.4.3')
      .get('input[name=ocp_sub_domain]').should('have.value', 'cypressi')
      .get('[aria-label="Persistent Storage Needs"]').select('50GB').should('have.value', '50G')

    cy.contains('Users').click()

    cy.contains('Add User').click()

    cy.get('input[name=last-name]').type('Lowry')
      .get('input[name=first-name]').type('Kyle')
      .get('input[name=email]').type('lowry@redhat.com')
      .get('[aria-label="User Role"]').select('Admin').should('have.value', 'admin')
